...
const createComment = asyncWrapper(async (req, res, next) => {
  ...
  const alreadycommentSubmitted = await bookmarksModel.findOne({
    post: postId,
    user: authUser._id,
  });
...
const updateComment = asyncWrapper(async (req, res, next) => {
  ...
  // const comment = await bookmarksModel.findById({ _id: commentId });
  // const comment = await bookmarksModel.findByIdAndUpdate(
  const comment = await bookmarksModel.findOneAndUpdate(
    {
      _id: commentId,
    },
    req.body,
